...
function Header() {
  ...
  return (
    <header className={`z-[999] transition-all duration-300 bg-black h-[102px]`}>
        ...
        <div className='w-[100%] md:w-52 h-16 flex items-center justify-between'>
          ...
          <div className="md:hidden cursor-pointer z-[10000]" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
            <div className="w-6 h-5 relative transition-all duration-300 ease-in-out">
              <span className={`absolute left-0 top-0 h-0.5 w-full bg-[#888888] transition-transform duration-300 ${mobileMenuOpen ? 'rotate-45 top-2.5' : ''}`} />
              <span className={`absolute left-0 top-2.5 h-0.5 w-full bg-[#888888] transition-opacity duration-300 ${mobileMenuOpen ? 'opacity-0' : 'opacity-100'}`} />
              <span className={`absolute left-0 bottom-0 h-0.5 w-full bg-[#888888] transition-transform duration-300 ${mobileMenuOpen ? '-rotate-45 bottom-2.5' : ''}`} />
            </div>
          </div>
        </div>

        {/* Desktop Menu */}
        <ul className='hidden lg:flex items-center gap-[32px] text-[14px] font-medium'>
          <li>
            <Link href="/studio" className={`${isActive('/studio') ? 'text-white' : 'text-[#888888]'} font-bold`}>Studio</Link>
          </li>
          <li>
            <Link href="/work" className={`${isActive('/work') ? 'text-white' : 'text-[#888888]'} font-bold`}>Work</Link>
          </li>
          <li>
            <Link href="/services" className={`${isActive('/services') ? 'text-white' : 'text-[#888888]'} font-bold`}>Services</Link>
          </li>
          <li>
            <Link href="/clients" className={`${isActive('/clients') ? 'text-white' : 'text-[#888888]'} font-bold`}>Clients</Link>
          </li>
          <li>
            <Link href="/contact" className={`${isActive('/contact') ? 'text-white' : 'text-[#888888]'} font-bold`}>Contact</Link>
          </li>
        </ul>
      </div>
      ...
      {/* Mobile Menu */}
      <div
        className={`fixed inset-0 z-[9999] shadow-lg bg-black h-[70%] transform transition-transform duration-500 ease-in-out ${mobileMenuOpen ? 'translate-y-0' : '-translate-y-full'
          }`}
      >
